<template>
  <div>
    ...
    >
      ...
      >
      <el-table-column label="请求参数" width="300" show-overflow-tooltip>
        <template v-slot="scope">
          {{ scope.row.params }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作结果" width="170">
        <template v-slot="scope">
          <el-button v-if="scope.row.result == 1" type="primary" round>{{ scope.row.resultName }}</el-button>
          <el-button v-if="scope.row.result == 0" type="warning" round>{{ scope.row.resultName }}</el-button>
        </template>
      </el-table-column>
